...
# Test Charlie, Dave, and Eve as External Validators (FIXED VERSION)
# This simulates the complete external validator onboarding process
# Based on the original VALIDATORS_SETUP_TESTNET_FENNELSOLONET.md approach

set -e

echo "👥 Testing Charlie, Dave & Eve as External Validators (FIXED)"
echo "============================================================"
...
# Port assignments for testing (matching old approach)
CHARLIE_PORT=9946
DAVE_PORT=9947
EVE_PORT=9948

# Function to start test validator (FIXED)
start_test_validator() {
    ...
    # Use simplified fennel-node arguments (matching old shell script approach)
    docker run -d \
        --name "$container_name" \
        -p "$port:9944" \
        -p "$((port + 100)):30333" \
...
# Function to generate session keys (matching old approach)
...
# Function to create validator registration summary (matching old approach)
create_registration_summary() {
    echo "📋 External Validator Registration Summary"
    echo "=========================================="
    echo ""
    echo "🔄 Process Overview (based on old validator setup):"
    ...
    echo "🎯 Step-by-Step Validator Registration (Old Approach):"
...
test_full_workflow() {
    echo "🎯 Testing Full External Validator Workflow (FIXED)"
    echo "=================================================="
...
show_menu() {
    echo ""
    echo "🎮 External Validator Testing Menu (FIXED)"
    echo "==========================================="
...
main() {
    echo "👥 External Validator Testing Tool (FIXED)"
    echo "=========================================="
    echo "This tool helps test Charlie, Dave & Eve as external validators"
    echo "Based on the original validator setup approach"
    echo ""
