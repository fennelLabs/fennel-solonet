...
# Function to test all bootnode types (from Parity docs)
test_all_bootnode_types() {
    print_status "Testing bootnode connectivity using Parity's recommended methods..."
    
    # Test P2P connection
    echo ""
    echo "=== Testing P2P Connection ==="
    local cluster_bootnode="/dns4/bootnode-0-relay-chain-p2p.fennel-staging.svc.cluster.local/tcp/30333/p2p/$(get_bootnode_peer_id)"
    test_bootnode_parity_method "$cluster_bootnode" "P2P"
    
    # Test external bootnodes (these should fail)
    echo ""
    echo "=== Testing External Bootnodes (Expected to Fail) ==="
    test_bootnode_parity_method "/dns4/bootnode1.fennel.network/tcp/30333/p2p/12D3KooWDCZGrnJhsgWJtDcs6eZc1hUBrVj5QqkEWggAkNVowRAi" "External-1" || true
    test_bootnode_parity_method "/dns4/bootnode2.fennel.network/tcp/30333/p2p/12D3KooWDbfFv6oepAXmQaiwFaVjD9g7AxowQ8LQdWTcVYpKhnAx" "External-2" || true
}
...
# Function to create deployment-ready chain specs
create_deployment_ready_chainspecs() {
    print_status "Creating deployment-ready chain specifications..."
    
    # Create deployment directory
    mkdir -p deployment-ready-chainspecs
    
    # Copy updated chainspecs to deployment directory
    for chainspec in chainspecs/staging/staging-chainspec.json chainspecs/development/development.json; do
        if [ -f "$chainspec" ]; then
            cp "$chainspec" "deployment-ready-chainspecs/$(basename "$chainspec")"
            
            # Generate raw version
            if command -v chain-spec-builder &> /dev/null; then
                local raw_name="deployment-ready-chainspecs/$(basename "$chainspec" .json)-raw.json"
                chain-spec-builder -c "$raw_name" convert-to-raw "$chainspec" || print_warning "Could not generate raw chainspec"
            fi
        fi
    done
    
    print_success "Deployment-ready chainspecs created in: deployment-ready-chainspecs/"
}

# Function to provide migration steps
provide_migration_steps() {
    echo ""
    echo "🚀 MIGRATION STEPS (Based on Parity Best Practices)"
    echo "=================================================="
    echo ""
    echo "1. 🔄 UPDATE CHAIN SPECIFICATIONS"
    echo "   - Backup created: chainspecs/*.backup-*"
    echo "   - Updated chainspecs with working bootnodes"
    echo ""
    echo "2. 📦 DEPLOY UPDATED CHAINSPECS"
    echo "   kubectl create configmap updated-chainspec \\"
    echo "     --from-file=deployment-ready-chainspecs/staging-chainspec.json \\"
    echo "     -n fennel-staging --dry-run=client -o yaml | kubectl apply -f -"
    echo ""
    echo "3. 🔄 RESTART NODES (Rolling Update)"
    echo "   kubectl rollout restart deployment/fennel-validators-node-0 -n fennel-staging"
    echo "   kubectl rollout restart deployment/fennel-validators-node-1 -n fennel-staging"
    echo "   kubectl rollout restart deployment/fennel-validators-node-2 -n fennel-staging"
    echo ""
    echo "4. ✅ VERIFY CONNECTIVITY"
    echo "   kubectl logs -f -n fennel-staging fennel-validators-node-0 | grep peers"
    echo ""
    echo "5. 🌐 OPTIONAL: Set up external bootnodes"
    echo "   - Deploy bootnodes with proper DNS (bootnode1.fennel.network)"
    echo "   - Follow Parity's bootnode setup guide"
    echo "   - Test with: $0 --test-external"
}
...
# Test current bootnode connectivity
test_all_bootnode_types
...
case "${1:-}" in
    ...
    --test-external)
        echo ""
        print_status "Testing external bootnode connectivity only..."
        test_bootnode_parity_method "/dns4/bootnode1.fennel.network/tcp/30333/p2p/12D3KooWDCZGrnJhsgWJtDcs6eZc1hUBrVj5QqkEWggAkNVowRAi" "External-1"
        test_bootnode_parity_method "/dns4/bootnode2.fennel.network/tcp/30333/p2p/12D3KooWDbfFv6oepAXmQaiwFaVjD9g7AxowQ8LQdWTcVYpKhnAx" "External-2"
        ;;
    --help)
        echo ""
        echo "Usage: $0 [OPTION]"
        ...
        echo "  --test-external Test external bootnode connectivity only"
        echo "  --help          Show this help message"
        echo ""
        echo "Based on official Parity documentation:"
        echo "  - https://docs.polkadot.com/infrastructure/running-a-node/setup-bootnode/"
        echo "  - https://paritytech.github.io/devops-guide/guides/readiness-checklist.html"
        ;;
    *)
        echo ""
...
echo ""
echo "📚 Next steps:"
echo "  1. Run: $0 --fix (to apply fixes)"
echo "  2. Follow the migration steps provided"
echo "  3. Monitor logs: kubectl logs -f -n fennel-staging bootnode-0" 